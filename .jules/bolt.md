...
## 2026-07-01 - [Algebraic Simplification of Scaled Array Operations]
**Learning:** When generating geometry using mathematical equations (like parabolas) based on a dynamically scaled domain `x = _X_NORMALIZED * L`, substituting `x` into the equation mathematically can sometimes cancel out dynamic variables (e.g. `L`) in portions of the array computation. In `MethodOfCharacteristics.solve()`, `A(x - L)**2` where `A = (rt - re)/L**2` simplified exactly to `(rt - re) * (_X_NORMALIZED - 1.0)**2`. This allowed moving the squared portion into a static module-level constant array, completely eliminating an `np.subtract()` and a squaring (`y *= y`) array operation dynamically, resulting in a ~35% speedup for contour generation.
**Action:** Before executing dynamic array operations (like subtraction and exponentiation) against sequences generated from static normalized arrays (`_X_NORMALIZED`), algebraically substitute the sequence generation formula to see if terms can be canceled or grouped into constant normalized components.
