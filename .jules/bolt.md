...
## 2026-06-20 - [Matplotlib LineCollection for Large Meshes]
**Learning:** In visualization methods like `MethodOfCharacteristics.plot_mesh()`, rendering thousands of individual line segments using a `for` loop of sequential `plt.plot()` calls, combined with intermediate list comprehension unzipping (`[-y for y in ly]`), creates massive Python iteration overhead and thousands of separate plot objects. Benchmarks showed plotting 50,000 lines taking ~79 seconds.
**Action:** Replace `for` loop `plt.plot()` calls with a single `matplotlib.collections.LineCollection` populated directly from NumPy arrays (e.g. `mesh_array`), completely eliminating O(N) iteration, memory reallocation, and list unzipping overhead. Using `LineCollection` dropped rendering time for 50,000 lines to <1s.

## 2026-06-25 - [Pre-computing Constant Normalized Array Layouts]
**Learning:** In heavily called array operations, repeatedly generating linear coordinate or ratio arrays dynamically using `np.arange(...) * step + offset` allocates a new base array (`np.arange`) during every execution. Benchmarks in `MethodOfCharacteristics.solve()` and `RocketPerformance.scan_mixture_ratio()` showed that pre-computing a static normalized layout array (`np.arange(size, dtype=float) / size`) as a module-level constant (`_X_NORMALIZED`) and reusing it to scale and offset inside the class methods reduces execution time. For 100-element arrays, coordinate generation overhead dropped from ~0.25s to ~0.14s per 100k calls (~45% faster).
**Action:** When a method frequently generates scaled geometry or stepped sequences of a constant size, pre-compute the normalized mapping array as a module-level constant. Multiply this constant array by the dynamically calculated total length/range to avoid repeated base array allocation overhead.

## 2026-06-30 - [Algebraically Simplify Array Operations by Cancelling Variables]
**Learning:** When calculating scaled array coordinates from a linear space (like `x = _X_NORMALIZED * length`) and substituting them into a polynomial equation (like `y = A*(x - L)^2 + re`), dynamic scaling variables (like `length`) may mathematically cancel out. In `MethodOfCharacteristics.solve()`, substituting `x` into the equation revealed that `length**2` canceled out completely. This allowed pre-computing the remaining normalized expression as a static array (`_NORMALIZED_PARABOLA = (_X_NORMALIZED - 1.0)**2`), replacing multiple dynamic array operations (subtraction, squaring, scaling) with a single array multiplication and improving coordinate generation speed by ~30%.
**Action:** Always write out the full mathematical formula when generating coordinates from a normalized layout array to see if dynamic scaling variables cancel out. If they do, pre-compute the remaining normalized expression as a static constant array.
