...
## 2026-07-01 - [Algebraic Substitution of Normalized Arrays]
**Learning:** When calculating scaled array coordinates from a linear space, algebraically substituting the normalized layout array into the final equation can reveal that dynamic scaling variables cancel out. By pre-computing the remaining normalized expression as a constant static array (e.g., `_NORMALIZED_PARABOLA = (_X_NORMALIZED - 1.0)**2`), we can replace multiple dynamic array operations (like subtracting, squaring, and scaling) with a single multiplication. This reduces execution time for coordinate generation by ~50%.
**Action:** Always attempt to algebraically substitute normalized layout arrays into final geometric equations. If the dynamic scale factors cancel out, pre-compute the remaining normalized expression as a static module-level constant to bypass redundant dynamic array math.
